...
class Ml {
    ...
    get(q, t) { return (!Type.isStr(q)) ? this.root : (Type.isEl(t) ? t: this.root).querySelector(q); }
    gets(q, t) { return [...(Type.isEl(t) ? t : this.root).querySelectorAll(q)] }
    add(el, ...children) {
        const frag = document.createDocumentFragment();
        for (let child of children.flat(Infinity)) { frag.append(child) }
        el.append(frag)
    }
    set(oldEl, ...newEls){oldEls.replaceWith(...newEls)}
    get css() { return CSS }
    get attr() { return ATTR }
    get on() { return ON }
    #handler(ns){return ({get:(_, name)=>this.#tag.bind(undefined, ns, name)})}
    #tag(ns, name, ...args) {
        const [props, ...children] = Type.isObj(args[0] ?? 0) ? args : [{}, ...args];
        const el = ns ? document.createElementNS(ns, name) : document.createElement(name)
        this.#mixin(el)
